...
import { getModelButtonLabel, resolveChatModelForPlan } from "@/lib/chatModels";
  ...
  return (
    <div className="relative w-full px-1 pb-4 pt-3 md:pb-5 md:pt-4">
      <div className={pageShellClass}>
        <div
          className={cn(
            "mx-0 flex w-full max-w-none flex-col gap-3 overflow-visible p-3",
            "backdrop-blur-md",
          )}
        >
          <div className="flex flex-col gap-3 p-0">
            <div className="flex flex-wrap items-center justify-between gap-2 px-1">
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                {contextLabel && (
                  <span className="border border-border bg-background/60 px-2 py-0.5 text-[10px] font-medium normal-case tracking-normal text-foreground">
                    {contextLabel}
                  </span>
                )}
                {!isPaid && (
            ...
            {disabled ? (
              <p className="border border-dashed border-border bg-muted-bg/30 px-3 py-2 text-xs text-muted">
                {mode === "research"
              ...
              <QuickAnalysisBar
                ...
                symbol={
                  mode === "portfolio" ? "PORTFOLIO" : (selectedSymbol ?? "")
                }
                loading={isBusy}
            ...
            >
              <textarea
                ...
                className={cn(
                  ...
                  "max-h-52 min-h-12 w-full resize-none bg-background/60 px-3 py-2 text-sm leading-relaxed text-foreground outline-none ring-1 ring-transparent transition placeholder:text-muted focus:ring-border disabled:cursor-not-allowed disabled:opacity-60",
                )}
                placeholder={
                  disabled
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  <button
                    ...
                    className={cn(
                      "inline-flex max-w-44 items-center gap-1 px-3 py-1.5 text-xs font-medium text-muted transition-all duration-200 ease-out hover:bg-muted-bg hover:text-foreground sm:max-w-none",
                      "disabled:opacity-60",
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 bg-foreground px-4 py-1.5 text-xs font-medium text-background transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {isChatLoading ? (
                      "Analyzing…"
                    ) : (
