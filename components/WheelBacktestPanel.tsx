    ...
    <div className={cn(isResearch ? appStackClass : "space-y-5", className)}>
      ...
      {result && (
        <>
            ...
            {(result.capitalTopUpsUsd ?? 0) > 0 && (
              <p className="rounded-lg border border-border bg-muted-bg/40 px-3 py-2.5 text-xs leading-relaxed text-foreground">
            ...
            <div className="rounded-xl bg-muted-bg/60 px-3 py-3 text-sm">
            ...
            {result.annualSummary.length > 0 && (
              <div>
                <p className={cn(appSectionLabelClass, "mb-3")}>Year by year</p>
                <div className={responsiveTableScrollClass}>
...
function CapitalKpi({
  ...
  return (
    <div className={cn(appKpiClass, "min-h-0 justify-start py-3")}>
      ...
      <p className={cn("mt-0.5 text-sm font-semibold tabular-nums", valueClassName)}>
        {value}
      </p>
...
function Metric({
  ...
  return (
    <CapitalKpi label={label} value={value} sub={sub} />
  );
}
