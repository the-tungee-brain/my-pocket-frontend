...
export function WatchlistHint({ symbol }: Props) {
  ...
  return (
    <div
      role="note"
      className="inline-flex max-w-full min-w-0 items-center gap-2 border border-accent/30 bg-accent-muted/30 py-2 pl-2.5 pr-1.5"
    >
      <Star
      ...
      <p className="m-0 min-w-0 flex-1 text-xs leading-snug text-muted">
        Star{" "}
        <span className="font-mono font-medium text-foreground">{upper}</span>{" "}
